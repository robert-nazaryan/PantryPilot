...
| `GEMINI_API_KEY` | Google AI Studio → API keys → create key (https://aistudio.google.com/apikey). | Powers the in-app AI chat assistant when `AI_PROVIDER=gemini`. If unset (empty), the `/api/ai/chat` endpoint responds with 503 `ai_unavailable` instead of crashing — the rest of the app runs normally. Model + base URL are overridable via `GEMINI_MODEL` and `GEMINI_API_BASE_URL` env vars if needed; defaults are `gemini-2.5-flash` and Google's public endpoint. |
| `GROQ_API_KEY` | Groq Cloud console → API Keys → Create API Key (https://console.groq.com/keys). | Powers the in-app AI chat assistant when `AI_PROVIDER=groq` (current default in `docker-compose.prod.yml`). Same fail-soft semantics as `GEMINI_API_KEY` — if unset, `/api/ai/chat` returns 503 `ai_unavailable`. Model + base URL overridable via `GROQ_MODEL` and `GROQ_API_BASE_URL`; defaults are `openai/gpt-oss-120b` and Groq's public endpoint. To switch providers, edit `AI_PROVIDER` in `docker-compose.prod.yml` (or set it in the workflow env / server `.env`). |
